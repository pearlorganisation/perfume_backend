...
export const createWriteReview = asyncHandler(async (req, res) => {
    ...
    {
        const review = new WriteReviewSchema({description,perfumeName,userId,images});
        await review.save();
...
export const updateWriteReview = asyncHandler(async (req, res) => {
    const review = await WriteReviewSchema.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
    if (!review) {
        return res.status(404).json({ message: 'Review not found' });
    }
    res.status(200).json(review);
...
export const approveWriteReview = asyncHandler(async (req, res) => {
    ...
    const review = await WriteReviewSchema.findByIdAndUpdate(req.params.id, {status:status}, { new: true, runValidators: true });
    if (!review) {
        return res.status(404).json({ message: 'Review not found' });
    }
    res.status(200).json(review);
